...
export default function AuthStatus({ isMobileMenu = false }: AuthStatusProps) {
    ...
    if (status === 'unauthenticated' || !session?.user) {
        ...
        if (isMobileMenu) {
            return (
                <div className="mt-3 pt-3 border-t border-default-200 flex flex-col space-y-2">
                    <Link
                        href="/auth/signin"
                        className="flex px-4 py-2.5 text-base text-gray-700 hover:bg-gray-100 rounded-lg transition-colors"
                    >
                        Sign In
                    </Link>
                    <Link
