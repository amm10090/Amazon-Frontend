    ...
    if (isLoading) {
        return (
            <div className="relative my-8 px-2 md:px-4">
                <h2 className="text-2xl font-bold mb-6 text-center bg-clip-text text-transparent bg-gradient-to-r from-primary to-secondary">Popular Categories</h2>
    ...
    if (error) {
        return (
            <div className="my-8 text-center px-4">
                <h2 className="text-2xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-primary to-secondary">Popular Categories</h2>
        ...
        <div className="relative my-8 px-4">
            ...
            <div className="relative">
                ...
                <AnimatePresence>
                    {isMobile && showLeftArrow && (
                        <motion.button
                            ...
                            exit={{ opacity: 0, x: 10 }}
                            className="absolute left-0 top-1/2 -translate-y-1/2 z-10 md:-left-4 bg-white/80 dark:bg-gray-800/80 backdrop-blur-md p-2 rounded-full shadow-md border border-gray-100 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:text-primary dark:hover:text-primary-light focus:outline-none focus:ring-2 focus:ring-primary/50 dark:focus:ring-primary-light/50"
                            onClick={scrollLeft}
                ...
                <AnimatePresence>
                    {isMobile && showRightArrow && (
                        <motion.button
                            ...
                            animate={{ opacity: 1, x: 0 }}
                            exit={{ opacity: 0, x: -10 }}
                            className="absolute right-0 top-1/2 -translate-y-1/2 z-10 md:-right-4 bg-white/80 dark:bg-gray-800/80 backdrop-blur-md p-2 rounded-full shadow-md border border-gray-100 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:text-primary dark:hover:text-primary-light focus:outline-none focus:ring-2 focus:ring-primary/50 dark:focus:ring-primary-light/50"
                            onClick={scrollRight}
                ...
                <div
                    ...
                    className="overflow-x-auto hide-scrollbar pb-4 pt-2 px-2 -mx-2"
                    style={{
                        ...
                        {categories.map((category, index) => (
                            ...
                            >
                                ...
                                >
                                    <motion.div
                                        className={`
                                            relative overflow-hidden rounded-xl h-40
                                            ...
                                            p-4 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col items-center justify-center
                                        `}
                                        whileHover={{
                                    ...
                                    >
                                        ...
                                        <motion.div
                                            className={`
                                                relative w-16 h-16 rounded-full mb-4 flex items-center justify-center 
                                                ${isActiveCategory(category.slug)
                                        ...
                                        >
                                            <motion.svg
                                                ...
                                                className={`h-8 w-8 ${isActiveCategory(category.slug) ? 'text-white' : 'group-hover:text-gray-700 dark:group-hover:text-gray-300'}`}
                                                fill="none"
                                                viewBox="0 0 24 24"
                                                stroke="currentColor"
                                                initial={{ opacity: 0.8 }}
                                            ...
                                            >
                                                {[...Array(3)].map((_, i) => (
                                                    <motion.div
                                                        key={i}
                                                        className={`absolute w-1.5 h-1.5 rounded-full bg-gradient-to-r ${category.color || 'from-gray-300 to-gray-400'}`}
                                                        initial={{
                                                            x: 0,
                                                            y: 0,
                                                            opacity: 0
                ...
                {isMobile && (
                    <div className="flex justify-center mt-4 space-x-1">
                        {categories.map((category, index) => (
                            <button
                                key={index}
                                className={`w-2 h-2 rounded-full transition-all duration-300 ${isActiveCategory(category.slug)
                                    ? 'bg-primary w-4'
                                    : 'bg-gray-300 dark:bg-gray-600 hover:bg-primary/50 dark:hover:bg-primary-light/50'
                                    }`}
                                onClick={() => {
                                    if (scrollContainerRef.current) {
                                        const cardWidth = 160; // 大约一张卡片的宽度
